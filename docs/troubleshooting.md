...
- Find a specific error in the output of the `excel-webview2-mcp` server.
  Usually, if your client is an IDE, logs would be in the Output pane.
- Search the [GitHub repository issues and discussions](https://github.com/dsbissett/excel-webview2-mcp) for help or existing similar problems.
...
`excel-webview2-mcp` and installing it again. Clear the cache by running:
...
### Windows 10: Error during discovery for MCP server 'excel-webview2': MCP error -32000: Connection closed
...
When installing `excel-webview2-mcp` as a Claude Code plugin (either from the
...
```
Failed to download/cache plugin excel-webview2-mcp: Failed to clone repository:
  Cloning into '...'...
  fatal: unable to access 'https://github.com/dsbissett/excel-webview2-mcp.git/':
...
If the plugin marketplace approach fails, you can install `excel-webview2-mcp`
