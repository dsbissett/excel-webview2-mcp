...
const y = yargs(hideBin(process.argv))
  .scriptName('excel-webview2')
  .showHelpOnFail(true)
  .usage('excel-webview2 <command> [...args] --flags')
  .usage(
    `Run 'excel-webview2 <command> --help' for help on the specific command.`,
  )
  .demandCommand()
  .version(VERSION)
  .strict()
...
y.command(
  'start',
  'Start or restart excel-webview2-mcp',
  y =>
    y
...
y.command('status', 'Checks if excel-webview2-mcp is running', async () => {
  if (isDaemonRunning()) {
    console.log('excel-webview2-mcp daemon is running.');
    ...
    console.log('excel-webview2-mcp daemon is not running.');
  }
  process.exit(0);
});

y.command('stop', 'Stop excel-webview2-mcp if any', async () => {
  if (!isDaemonRunning()) {
    process.exit(0);
  }
  await stopDaemon();
