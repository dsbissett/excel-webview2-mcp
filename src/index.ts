...
export async function createMcpServer(
  ...
  const server = new McpServer(
    {
      name: 'excel_webview2',
      title: 'Excel WebView2 MCP server',
      version: VERSION,
    },
  ...
  async function getContext(): Promise<McpContext> {
    ...
    // Connecting via browserUrl targets WebView2; wsEndpoint/autoConnect target Chrome.
    const isWebView2Mode = Boolean(
    ...
    if (context?.browser !== browser) {
      context = await McpContext.from(browser, logger, {
        ...
        // In WebView2 mode, include all target types (webview, about:blank, etc.)
        // unless the user has explicitly set the flag.
        experimentalIncludeAllPages:
          serverArgs.experimentalIncludeAllPages ?? isWebView2Mode,
...
export const logDisclaimers = (args: ReturnType<typeof parseArguments>) => {
  console.error(
    `excel-webview2-mcp exposes content of the browser instance to the MCP clients allowing them to inspect,
...
Avoid sharing sensitive or personal information that you do not want to share with MCP clients.`,
  ...
  if (!args.slim && args.usageStatistics) {
    console.error(
      `
Google collects usage statistics to improve Excel WebView2 MCP. To opt-out, run with --no-usage-statistics.
For more details, visit: https://github.com/dsbissett/excel-webview2-mcp#usage-statistics`,
    );
  }
};
