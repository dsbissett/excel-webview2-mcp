...
import type {ExcelWebview2McpExtension} from '../../src/telemetry/types';

const SERVER_PATH = path.resolve('build/src/bin/excel-webview2-mcp.js');

interface MockServerContext {
  server: http.Server;
  port: number;
  events: ExcelWebview2McpExtension[];
  watchdogPid?: number;
  waitForEvent: (
    predicate: (event: ExcelWebview2McpExtension) => boolean,
  ) => Promise<ExcelWebview2McpExtension>;
}

async function startMockServer(): Promise<MockServerContext> {
  const events: ExcelWebview2McpExtension[] = [];
  let waitingResolvers: Array<{
    predicate: (event: ExcelWebview2McpExtension) => boolean;
    resolve: (event: ExcelWebview2McpExtension) => void;
  }> = [];
