...
export const CLI_PATH = path.resolve('build/src/bin/excel-webview2.js');
...
export async function assertDaemonIsNotRunning() {
  ...
  assert.strictEqual(
    result.stdout,
    'excel-webview2-mcp daemon is not running.\n',
  );
}
...
export async function assertDaemonIsRunning() {
  const result = await runCli(['status']);
  assert.ok(
    result.stdout.startsWith('excel-webview2-mcp daemon is running.\n'),
    'excel-webview2-mcp daemon is not running',
  );
}
