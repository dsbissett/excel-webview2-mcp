...
import {cliOptions} from '../build/src/bin/excel-webview2-mcp-cli-options.js';
import type {ParsedArguments} from '../build/src/bin/excel-webview2-mcp-cli-options.js';
...
async function measureServer(args: string[]) {
  ...
  const transport = new StdioClientTransport({
    command: 'node',
    args: ['./build/src/bin/excel-webview2-mcp.js', ...args], // Point to your built MCP server
  });
  ...
  try {
...

    {
      ...
      await generateReference(
        'Excel WebView2 MCP Tool Reference',
        OUTPUT_PATH,
        toolsWithAnnotations,
        categories,
        sortedCategories,
...

    {
      ...
      await generateReference(
        'Excel WebView2 MCP Slim Tool Reference',
        SLIM_OUTPUT_PATH,
        toolsWithAnnotations,
        categories,
        sortedCategories,
